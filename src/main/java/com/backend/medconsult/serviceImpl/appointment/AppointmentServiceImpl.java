...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public AppointmentResponseDto bookAppointment(AppointmentRequestDto dto,
        ...
        DoctorClinic doctorClinic = doctorClinicRepository.findById(dto.getDcId())
                .orElseThrow(() -> new RuntimeException("Doctor-Clinic association not found with ID: " + dto.getDcId()));
        ...
        AppointmentSlot slot = appointmentSlotRepository.findById(dto.getSlotId())
                .orElseThrow(() -> new RuntimeException("Appointment slot not found with ID: " + dto.getSlotId()));

        // 4. Validate slot availability
        if (slot.getStatus() != SlotStatus.AVAILABLE) {
            throw new RuntimeException("The selected slot is no longer available (status: " + slot.getStatus() + ")");
        }
        ...
        if (!slot.getSlotDate().equals(dto.getScheduledDate())) {
            throw new RuntimeException("Scheduled date does not match the slot date (" + slot.getSlotDate() + ")");
        }
    ...
    public List<AppointmentResponseDto> getMyUpcomingAppointments(CustomUserPrincipal authUser,
        ...
        Patient patient = patientRepository.findByUser_UserId(user.getUserId())
                .orElseThrow(() -> new RuntimeException("No patient profile found for the current user"));
