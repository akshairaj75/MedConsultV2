...
public class LabItemResponseDto {
    ...
    public static LabItemResponseDto fromEntity(LabItem item) {
        if (item == null) {
            return null;
        }
        LabItemResponseDto dto = new LabItemResponseDto();
        dto.setItemId(item.getItemId());
        if (item.getLabResult() != null) {
            dto.setLabResultId(item.getLabResult().getLabResultId());
        }
        dto.setTestName(item.getTestName());
        dto.setLoincCode(item.getLoincCode());
        dto.setValue(item.getValue());
        dto.setUnit(item.getUnit());
        dto.setReferenceLow(item.getReferenceLow());
        dto.setReferenceHigh(item.getReferenceHigh());
